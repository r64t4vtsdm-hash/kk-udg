...
function LegList({
  ...
  function update(index: number, key: keyof Fixture, value: string) {
    onChange(items.map((it, i) => (i === index ? { ...it, [key]: value } : it)));
  }
  function add() {
    onChange([...items, { date: "", opponent: "" }]);
  }
...
export default function FixturesEditor({
  ...
  return (
    <div>
      <LegList legLabel="Jesenji dio" items={firstLeg} onChange={setFirstLeg} />
      <LegList legLabel="Proljećni dio — revanš" items={secondLeg} onChange={setSecondLeg} />
